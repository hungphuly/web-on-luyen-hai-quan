...
- **Cloudflare Workers & Environment Variables**: 
  - To prevent Next.js Webpack from statically replacing `process.env.VAR` with `undefined` at build time, always assign `process.env` to a variable first: `const env = process.env; const myVar = env['VAR'];`.
  - When deploying via Cloudflare Workers Builds, plain-text environment variables added manually via the Dashboard are wiped out on the next deploy. They MUST be declared in `wrangler.toml` under the top-level `[vars]` block.
  - Secrets MUST be added via the CLI (`npx wrangler secret put SECRET_NAME`) and MUST NOT be hardcoded in `wrangler.toml`. Cloudflare will persist secrets across deploys.
