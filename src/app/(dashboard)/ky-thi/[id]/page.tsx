...
export default async function TakeExamPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { session, kyThi, error } = await getOrCreateKyThiSession(id);
  ...
  if (error || !session || !kyThi) {
    return (
      <div className="max-w-3xl mx-auto mt-10 p-6 bg-red-50 text-red-700 rounded-xl border border-red-200">
        ...
        <p>{error}</p>
        <Link href="/ky-thi" className="inline-block mt-4 px-4 py-2 bg-red-600 text-white rounded-lg font-semibold hover:bg-red-700 transition">
          Quay lại danh sách
        </Link>
      </div>
    );
