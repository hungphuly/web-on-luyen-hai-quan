...
export async function nopBaiThiThu(
  ...
  const { data: ketQua, error: insertError } = await supabaseAdmin
    ...
    .insert({
      ...
      loai_bai: 'thi_thu',
      diem_so: diemSoStr as any, // Cột diem_so hiện tại là numeric, wait...
  ...
  // CHÚ Ý: Cột diem_so trong DB là `numeric`. Nếu lưu "x/20" sẽ lỗi type cast!
  // Sửa lại: Lưu `diem_so` là kiểu số `soCauDung`. Chúng ta có thể biết tổng số qua array length.
  ...
  if (insertError) {
    ...
    if (insertError2) {
      ...
      throw new Error('Lỗi khi lưu kết quả thi');
    }
